...
export default {
    ...
    setup() { 
        ...
        const gettype = () => {
            $.ajax({
                url: "http://" + store.state.url.ip + ":" + store.state.url.port + "/api/teacher/exercise/type/get/",
                type: "post",
                headers: {
        ...
        const updatetype = () => { 
            $.ajax({
                url: "http://" + store.state.url.ip + ":" + store.state.url.port + "/api/teacher/exercise/type/modify/",
                type: "post",
                headers: {
                    Authorization: "Bearer " + store.state.user.token,
                },
        ...
        const deletetype = rowdata => { 
            $.ajax({
                url: "http://" + store.state.url.ip + ":" + store.state.url.port + "/api/teacher/exercise/type/delete/",
                type: "post",
                headers: {
                    Authorization: "Bearer " + store.state.user.token,
                },
                ...
                success(resp) {
                    if (resp.error_message === "success") {
                        ...
                        ElMessage.success("删除类型失败，请检查你是否具有权限");
                        console.log(resp);
                    }
                },
                error(resp) {
